    ...
    if($METHOD == "POST"){
        ...
        if($result){
            ...
            if($create_table){
                ...
                $message = "Your api key is: " . $key . "\nDo not lose this key.  This key will expire in 7 days on " . date('l jS \of F Y h:i:s A') . "In order to prevent spam, only one key can be allocated for every email address.  If you would like to renew this key please send a request to kelseywilliams1200@gmail.com\nRead the documentation on the kelseywilliams.net api homepage for details on connecting to and using the api. ";
                $headers = "From:" . $from;
